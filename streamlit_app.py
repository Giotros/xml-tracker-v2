...
# --- UI: Supplier Filtering ---
st.subheader("🏭 Επίλεξε Προμηθευτή")
# Ελέγχουμε αν η στήλη 'supplier' υπάρχει στα δεδομένα
if "supplier" in df_history.columns:
    # Δημιουργούμε τη λίστα προμηθευτών, με την επιλογή "Όλοι" στην αρχή
    suppliers = ["Όλοι"] + sorted(df_history["supplier"].unique().tolist())
    selected_supplier = st.selectbox("Προμηθευτής", suppliers)

    # Φιλτράρουμε τα δεδομένα αν ο χρήστης επιλέξει έναν συγκεκριμένο προμηθευτή
    if selected_supplier != "Όλοι":
        df_history = df_history[df_history["supplier"] == selected_supplier]
else:
    # Εμφανίζουμε μήνυμα αν η στήλη δεν έχει δημιουργηθεί ακόμα
    st.info("Δεν βρέθηκε η στήλη 'supplier'. Εκτελέστε το tracker για να προστεθεί.")
